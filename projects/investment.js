    ...
    const nodes = {
        me: {
            ...
            body: {
                ...
                zh: '唯一的人类用户就是我。Dashboard 是一个单页应用——Alpine.js + Tailwind 写在一个 HTML 模板里——从 Flask API 拉 JSON。页头并排放着悉尼和纽约两个时钟，因为这个 app 同时活在两个时区里。'
            },
            why: {
                ...
                zh: '对单个用户，模板 + Alpine 胜过一套 React 工程：没有打包器、没有构建步骤、一个文件读完。前端专注于真正该做的事：清楚地呈现资金状况。'
            },
        ...
        flask: {
            ...
            body: {
                ...
                zh: '一个 app-factory 模式的 Flask 应用，8 个 API blueprint——持仓、交易、自选、行情、盈亏、搜索、资金池、元信息——跑在 gunicorn 后面，/api/health 做健康检查。自定义域名经 Cloudflare DNS 指到这里。写操作需要 bearer token，读保持开放。'
            },
            why: {
                ...
                zh: '它最早跑在我的笔记本上，靠 Cloudflare Tunnel 暴露、6 个 launchd 代理维持。迁到 Railway 之后整层运维都删掉了——合上 MacBook 盖子面板也不会下线，部署就是一次 git push。'
            },
        ...
        db: {
            ...
            body: {
                ...
                zh: '一个开着 WAL 模式的 SQLite 文件，放在 Railway 的持久 /data 卷上。交易账本是唯一的录入入口：买入更新加权平均成本，卖出累计已实现盈亏，每个美东交易日一行日盈亏记录，喂给日历。'
            },
            why: {
                ...
                zh: '单用户、单写入者——SQLite 无敌。没有数据库服务、没有网络开销，迁移是幂等的增量语句（PRAGMA 检查后再加列），永远不会弄坏运行中的应用。整个数据库就是卷上的一个文件。'
            },
        ...
        upstreams: {
            ...
            body: {
                ...
                zh: '三类行情数据：一次批量、GBK 解码的腾讯 qt 调用同时覆盖所有 A 股和港股代码；yfinance 负责美股、澳股和汇率；akshare 拉内地基金净值和上海金。每个源都有包裹——失败只会让对应行显示"无价格"，不会中断整次刷新。'
            },
            why: {
                ...
                zh: '用免费库，不用付费数据商——对个人追踪足够了。批量腾讯调用取代了原来 akshare 翻 14 页、耗时约 40 秒的抓取；现在整个 A 股 + 港股账本一次请求刷完。'
            },
            flows: ['arch-line-external']
        },
        sched: {
            eyebrow: { en: 'NODE · CRON', zh: '节点 · 定时任务' },
            title:   { en: 'APScheduler — exactly two jobs', zh: 'APScheduler——正好两个任务' },
            body: {
                ...
                zh: '跑在同一个容器里的进程内 cron：每 5 分钟刷新行情和汇率；工作日纽约时间 16:15——美股收盘 15 分钟后——计算当日盈亏。交易日以美东为准，我看到的一切按悉尼时间显示。'
            },
            why: {
                ...
                zh: '只有一个容器，就用进程内调度：不用多部署任何东西，max_instances=1 防止任务重叠。"双时钟"模型（美东交易日、悉尼显示）是整个 app 最微妙的逻辑——它让"今天"对我和对市场是同一个意思。'
            },
        ...
        agent: {
            ...
            body: {
                ...
                zh: '每个工作日北京时间 09:20——A 股开盘前十分钟——一个定时 Claude agent（Cowork + Chrome 插件）读取 /api/holdings、/api/buckets、/api/watchlist 和 /api/summary，给我写一份盘前简报，且必须遵守几条它无权推翻的硬性风控规则。'
            },
            why: {
                ...
                zh: '这就是 API 做成 REST、读保持开放的原因：这个面板从设计上就同时服务人和 agent。渲染我屏幕的那份 JSON，同时也在给我的早晨做简报——不用抓取，不用复制粘贴。'
            },
